...
deploy_kardinal_manager() {
    ...
    # Check if UUID file exists and is not empty
    if [ -s "$UUID_FILE" ]; then
        ...
        local deploy_output
        deploy_output=$(docker run --rm -v ${PWD}:/workdir -v /var/run/docker.sock:/var/run/docker.sock -w /workdir --network host --entrypoint $KARDINAL_CLI_PATH kurtosistech/kardinal-cli deploy -d voting-app-demo/compose.yml 2>&1)
